...
(script command_script rec_sen_01
	(cs_enable_pathfinding_failsafe true)
	(cs_enable_targeting true)
	(cs_fly_to rec_sen_01/p2)
;	(cs_fly_to rec_sen_01/p3)
	(cs_run_command_script rec_center_sentinels_01/left abort)
)

(script command_script rec_sen_02
	(cs_enable_pathfinding_failsafe true)
	(cs_enable_targeting true)
	(cs_fly_to rec_sen_01/p0)
;	(cs_fly_to rec_sen_01/p1)
	(cs_run_command_script rec_center_sentinels_01/right abort)
)
...
(global object chatter_grt_01 none)
(script command_script heretic_chat
	(cs_switch chat_grt_01)
	(cs_abort_on_alert TRUE)
	(set chatter_grt_01 (ai_get_object rec_center_heretic_01/g01))
	...
	(cs_abort_on_alert TRUE)
	(set chatter_her_01 (ai_get_object rec_center_heretic_01/h01))
	(cs_enable_pathfinding_failsafe TRUE)
...
;	(cs_switch chat_grt_01)
;	(cs_enable_pathfinding_failsafe TRUE)
;	(cs_approach chatter_her_01 1 20 20)
	...
	(cs_play_line 0320)
	(sleep 30)
	(cs_approach chatter_grt_01 1.5 20 20)
	(cs_suppress_dialogue_global TRUE)
	(cs_play_line 0330)
	
	(cs_switch chat_grt_01)
	(cs_face_object TRUE (ai_get_object rec_center_heretic_01/h01))
...
;	(sleep 60)
	...
	(cs_switch chat_grt_01)
	(cs_face_object FALSE (ai_get_object rec_center_heretic_01/h01))
	(cs_go_to recycling_center/p5)
	(cs_suppress_dialogue_global TRUE)
	(cs_play_line 0350)
	(sleep 30)
...
;restored Sentinel reinforce sequence ~'Vadam
(script dormant rec_center_sentinel_reinforce
	...
	(sleep (ai_play_line rec_center_h_entry 0450))
	(sleep 90)
...
(script dormant recycling_center_start
	...
	(ai_place rec_center_heretic_01 2)
...
;Sentinel Emitter encounter stuff ~'Vadam
...
(global boolean sen_grunt_line1_done FALSE)
(global boolean sen_elite_line1_done FALSE)
(global boolean sen_no_bitches FALSE)
(global boolean sen_spawns_started FALSE)
(global boolean sen_elite_line2_done FALSE)
(global boolean sen_grunt_line2_done FALSE)
(global boolean sen_elite_line3_done FALSE)
(global boolean sen_done FALSE)

(script command_script sen_emit_grunt_react
	(cs_switch grunt)
	(sleep 15)
	(cs_play_line 0400)
	(set sen_grunt_line1_done TRUE)
	(sleep_until (= sen_elite_line1_done TRUE))
	(sleep_until (= sen_spawns_started TRUE))
	(cs_enable_moving true)
	(cs_enable_targeting true)
	(sleep_until (= sen_elite_line2_done TRUE))
	(cs_play_line 0420)
	(set sen_grunt_line2_done TRUE)
	(sleep_until (= sen_elite_line3_done TRUE))
	(cs_play_line 0440)
)

(script command_script sen_emit_elite_react
	(cs_switch elite)
	(sleep_until (= sen_grunt_line1_done TRUE))
	(cs_play_line 0390)
	(set sen_elite_line1_done TRUE)
	(sleep_until (= sen_spawns_started TRUE))
	(cs_enable_moving true)
	(cs_enable_targeting true)
	(sleep 45)
	(cs_play_line 0410)
	(set sen_elite_line2_done TRUE)
	(sleep_until (= sen_grunt_line2_done TRUE))
	(cs_play_line 0430)
	(set sen_elite_line3_done TRUE)
)

(script command_script underhangar_sen_patrol1
	(cs_enable_pathfinding_failsafe TRUE)
	(cs_abort_on_alert true)
	(cs_abort_on_damage true)
	(cs_fly_by underhangar_sen_init/p6)
	(cs_shoot_point true underhangar_sen_init/weld1)
	(sleep_forever)
)

(script command_script underhangar_sen_patrol2
	(cs_enable_pathfinding_failsafe true)
	(cs_abort_on_alert true)
	(cs_abort_on_damage true)
	(begin_random
		(begin
			(cs_fly_to underhangar_sen_init/p7)
			(sleep 60)
		)
		(begin
			(cs_fly_to underhangar_sen_init/p5)
			(sleep 60)
		)
		(begin
			(cs_fly_to underhangar_sen_init/p10)
			(sleep 60)
		)
	)
)

(script command_script underhangar_sen_patrol3
	(cs_abort_on_alert true)
	(cs_abort_on_damage true)
	(cs_shoot TRUE underhangar_emit_04)
	(sleep_forever)
)

(script command_script underhangar_sen_patrol4
	(cs_enable_pathfinding_failsafe true)
	(cs_abort_on_alert true)
	(cs_abort_on_damage true)
	(begin_random
		(begin
			(cs_fly_to underhangar_sen_init/p4)
			(sleep 60)
		)
		(begin
			(cs_fly_to underhangar_sen_init/p8)
			(sleep 60)
		)
		(begin
			(cs_fly_to underhangar_sen_init/p9)
			(sleep 60)
		)
	)
)

(script dormant underhangar_sentinels_loop
	(sleep_until (= sen_spawns_started TRUE))
	(cond
	...
	(sleep_until
		(begin
			(sleep_until (<= (ai_living_count underhangar_sentinels) g_insertion_limit))
			...
			(sleep_until ; wave loop 
				(begin
					(ai_place underhangar_sentinels 1)
...
(script dormant underhangar_sentinels_main
	(sleep_until (> (ai_combat_status underhangar_sentinels_initial) ai_combat_status_uninspected))
	(unit_add_equipment (unit (list_get (ai_actors underhangar_sentinels_initial) 0)) swap TRUE TRUE)
...
(script dormant underhangar_firsttime_start
	...
	(wake underhangar_sentinels_main)
	(wake underhangar_sentinels_loop)
...
;	(ai_place underhangar_grunts_01 3)
	...
	(wake new_allies_02)
	(sleep_until (= (ai_living_count underhangar_sentinels_initial) 0))
	(unit_open underhangar_emit_01)
	(object_cannot_take_damage underhangar_emit_01)
	(unit_open underhangar_emit_02)
	(object_cannot_take_damage underhangar_emit_02)
	(unit_open underhangar_emit_03)
	(object_cannot_take_damage underhangar_emit_03)
	(unit_open underhangar_emit_04)
	(object_cannot_take_damage underhangar_emit_04)
	(unit_open underhangar_emit_05)
	(object_cannot_take_damage underhangar_emit_05)
	
	(ai_scene underhangar_sen_grunt_react_scene sen_emit_grunt_react allies_grunts)
	...
	(sleep_until (= sen_spawns_started TRUE))
	(object_can_take_damage underhangar_emit_01)
	(object_can_take_damage underhangar_emit_02)
	(object_can_take_damage underhangar_emit_03)
	(object_can_take_damage underhangar_emit_04)
	(object_can_take_damage underhangar_emit_05)
	
	(sleep_until
		(and
			(= (object_get_health underhangar_emit_01) 0)
			(= (object_get_health underhangar_emit_02) 0)
			(= (object_get_health underhangar_emit_03) 0)
			(= (object_get_health underhangar_emit_04) 0)
			(= (object_get_health underhangar_emit_05) 0)
			(< (ai_living_count underhangar_sentinels) 2)
		)
	)
	(set sen_done TRUE)
	(device_operates_automatically_set underhangar_exit TRUE)
	(sleep_forever underhangar_sentinels_loop)
...
(script command_script heretic_leader_01_escape
	(cs_force_combat_status 3)
	(sleep 10)
	(cs_go_to bottling_plant0/p0)
	(ai_erase ai_current_actor)
)
...
;	(ai_place second_hall_heretic_05)
	...
	(ai_place heretic_leader_01)
	(object_cannot_take_damage (ai_actors heretic_leader_01))
...
(script dormant fuck_this_turret_shit_1
	(ai_vehicle_enter dog_turret_men_03/left (ai_vehicle_get_from_starting_location dog_turrets_03/left))
	(ai_vehicle_enter dog_turret_men_03/right (ai_vehicle_get_from_starting_location dog_turrets_03/right))
)

(script dormant fuck_this_turret_shit_2
	(ai_vehicle_enter dog_turret_men_06/left (ai_vehicle_get_from_starting_location dog_turrets_06/left))
	(ai_vehicle_enter dog_turret_men_06/right (ai_vehicle_get_from_starting_location dog_turrets_06/right))
)
